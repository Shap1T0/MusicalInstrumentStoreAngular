...
namespace MusicalInstrumentStore
{
    public class Startup
    {
        ...
        {
            //string con = "Server=DESKTOP-6LKSKRB\\SQLEXPRESS;Database=MusicalInstrumentStore;Trusted_Connection=true;";
            //services.AddDbContext<Context>(options => options.UseSqlServer(con));
            services.AddControllers();
            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "MusicalInstrumentStore", Version = "v1" });
            });
